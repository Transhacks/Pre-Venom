	...
	public void onBlockPlace(BlockPlaceEvent event) {
		if (!RegionManager.getInstance().locationIsInRegion(event.getBlockPlaced().getLocation(), RegionType.SPAWN)) {
			if (event.getBlockPlaced().getType() == Material.OBSIDIAN) {
				this.obsidianToRemovalTasks.put(event.getBlockPlaced(),
						Bukkit.getServer().getScheduler().scheduleSyncRepeatingTask(Main.INSTANCE, () -> {
							this.removalTime.put(event.getBlockPlaced(),
									(Integer) this.removalTime.getOrDefault(event.getBlockPlaced(), 120) - 1);
							if ((Integer) this.removalTime.get(event.getBlockPlaced()) <= 0) {
								Integer taskId = this.obsidianToRemovalTasks.get(event.getBlockPlaced());
								if (taskId != null) {
			...
			while (var2.hasNext()) {
				...
				if (obsidianBlocks.equals(event.getBlock())) {
					Bukkit.getServer().getScheduler()
							.cancelTask((Integer) this.obsidianToRemovalTasks.get(event.getBlock()));
